...
impl RuntimeAssetManager {
    ...
    pub fn remove_image(&mut self, asset_id: AssetId) -> bool {
        self.image_generations.remove(&asset_id).is_some()
    }

    #[must_use]
    pub fn validate_image_decode(&self, result: ImageDecodeResult) -> ImageDecodeValidation {
        ...
        match result.result {
            Ok(image) => ImageDecodeValidation::Ready(ValidatedDecodedImage {
                asset_id: result.asset_id,
                generation: result.generation,
                path: result.path,
                image,
            }),
            Err(error) => ImageDecodeValidation::Failed(CurrentImageDecodeFailure {
                asset_id: result.asset_id,
                generation: result.generation,
                path: result.path,
                error,
...
#[cfg(test)]
mod tests {
    use super::{ImageDecodeValidation, RuntimeAssetManager};
    use crate::image_decode::{
        DecodedImage, ImageDecodeError, ImageDecodeGeneration, ImageDecodeResult,
    };
    use rhythm_core::ids::AssetId;
